...
levels = Counter(rows.values())
nonzero = sum(v for k, v in levels.items() if k > 0)
print(f'calendar cells={len(rows)} level-counts={dict(sorted(levels.items()))} nonzero-days={nonzero}')
if len(rows) < 300:
    raise SystemExit(f'profile contribution calendar not found: {len(rows)} cells')
...
for x in range(weeks):
    for y in range(7):
        ...
        px = 6 + x * (CELL + GAP)
        py = 28 + y * (CELL + GAP)
        svg.append(f'<rect class="c" x="{px}" y="{py}" width="{CELL}" height="{CELL}" rx="2" fill="{COLORS[level]}" style="animation-delay:{delay}ms"/>')
points = ' '.join(f'{6 + x * (CELL + GAP) + CELL / 2},{28 + y * (CELL + GAP) + CELL / 2}' for x, y in order)
svg.append(f'<polyline id="path" points="{points}" fill="none" stroke="none"/>')
for i in range(5):
    color = '#ffffff' if i == 0 else '#a371f7'
    radius = 4 if i == 0 else 3
    svg.append(f'<circle r="{radius}" fill="{color}"><animateMotion dur="14s" begin="{-i * 180}ms" repeatCount="indefinite"><mpath href="#path"/></animateMotion></circle>')
svg.append('</svg>')
